doChangeVersion(){
	...
	tgt_env_name=$(echo $tgt_env_name | perl -ne "s/$ENV/dev/g;print") # ALWAYS !!!
	tgt_PRODUCT_DIR=$product_dir/$tgt_env_name
	mkdir -p $tgt_PRODUCT_DIR	

	test "$tgt_PRODUCT_DIR" == "$PRODUCT_DIR" && return
	rm -fvr $tgt_PRODUCT_DIR/*
	test $? -eq 0  || do_exit 2 "cannot write to $tgt_PRODUCT_DIR !"
	
	doCreateRelativePackage
	unzip -o $zip_file -d $tgt_PRODUCT_DIR ; cp -v $zip_file $tgt_PRODUCT_DIR
   ...
   export dir_to_morph=$tgt_PRODUCT_DIR/cnf
   doMorphDir # :fin morph-dir.func.sh
	...
	while read -r file ; do (
		...
		do_log "DEBUG to tgt file: $tgt_PRODUCT_DIR/$file"
		test -f "$PRODUCT_DIR/$file" \
			&& diff "$PRODUCT_DIR/$file" "$tgt_PRODUCT_DIR/$file"
		[[ $? != 0 ]] && exit $?
	);
