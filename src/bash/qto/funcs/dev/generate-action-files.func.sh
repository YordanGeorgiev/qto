...
doGenerateActionFiles(){
	...
	do_log "DEBUG START : doGenerateActionFiles"
	
	# for each defined action 	
	while read -r act ; do (

		do_log "DEBUG START :: checking action: $act"
			# for each defined action 	
			while read -d" " deliverable_type ; do (

				do_log "DEBUG START ::: action: $act - deliverable_type: $deliverable_type "
				# src: https://gist.github.com/tyru/358703
				func_part_name=$(echo $act|perl -ne 's{(\w+)}{($a=lc $1)=~s<(^[a-z]|-[a-z])><($b=uc$1);$b;>eg;$a;}eg;s|-||g;print')
				do_log "DEBUG func_part_name: $func_part_name"
				...
				do_log " DEBUG full_func: $full_func"
				do_log " INFO delvr_doc_file: $deliverable_doc_file"
				do_log " INFO delvr_code_file: $deliverable_code_file"
				do_log " DEBUG STOP  ::: action: $act - deliverable_type: $deliverable_type "
