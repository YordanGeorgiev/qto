...
doRunTests(){
   ...
   $PRODUCT_DIR/src/bash/qto/tests/run-qto-tests.lst"
   do_log "INFO running the following actions : "
   cat $PRODUCT_DIR/src/bash/qto/tests/run-qto-tests.lst | grep -v '#'
   sleep 1
	...
	done < <(cat $PRODUCT_DIR/src/bash/qto/tests/run-qto-tests.lst)
...
do_log(){
   ...
   test -z $log_file && \
		mkdir -p $PRODUCT_DIR/dat/log/bash && \
			log_file="$PRODUCT_DIR/dat/log/bash/$RUN_UNIT_TESTER.`date "+%Y%m"`.log"
...
doSetVars(){
   ...
   source "$PRODUCT_DIR/src/bash/qto/funcs/log-test-run-entry.func.sh"
   ...
   ENV=$(echo `basename "$PRODUCT_DIR"`|cut -d'.' -f5)
   product_version=$(echo `basename "$PRODUCT_DIR"`|cut -d'.' -f2-4)
...
doParseConfFile(){
	...
	test -f "$PRODUCT_DIR/$RUN_UNIT_TESTER.$host_name.cnf" \
		&& cnf_file="$PRODUCT_DIR/$RUN_UNIT_TESTER.$host_name.cnf"
