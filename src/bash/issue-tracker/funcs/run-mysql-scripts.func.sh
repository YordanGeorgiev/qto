...
doRunMySqlScripts(){
   ...
   msg="the return value ret will be non-zero if the db has not been created"
   ...
   test $ret -ne 0 && doExit $exit_code "failed to create the mysql db : $mysql_db_name"
   test $ret -ne 0 && break
   ...
   echo -e "\n\n"
	...
	doLog "INFO "$(find "$mysql_scripts_dir" -type f -name "*.sql"|sort -n)
   ...
   while read -r sql_script ; do 
      ...
      mysql -t -u"$mysql_user" -p"$mysql_user_pw" \
         -D"$mysql_db_name" < "$sql_script" > "$tmp_log_file" 2>&1
      ret=$?
