...
do_provision_nginx(){
   ...
   sudo cp -v $DEPLOY_DIR/cnf/nginx/etc/nginx/nginx.conf /etc/nginx/nginx.conf

   source $DEPLOY_DIR/lib/bash/funcs/export-json-section-vars.sh
   for env in `echo dev tst prd`; do \

      doExportJsonSectionVars $DEPLOY_DIR/cnf/env/$env.env.json '.env.app'
      sudo cp -v $DEPLOY_DIR/cnf/nginx/etc/nginx/sites-available/%env%.http-site.conf \
   ...
   doExportJsonSectionVars $DEPLOY_DIR/cnf/env/$ENV_TYPE.env.json '.env.app'
