...
sub doConvert {
   ...
   foreach my $key ( sort  {$a<=>$b} keys %$hsr2_in  ) {
      ...
      if ( !defined ( $level ) ) {
         $msg = " the level for row id $rid was not defined !!! Error found in the following table: $table" ; 
         carp $msg ; 
         return ( $ret , $msg ) ; 
      } 
      ...
      if ( $rid == 0 ) {
         if ( $level ne $levelh ) {
            $msg = "the header for the level should always be : 
               level for postgres 
               and Level for mysql. Error found in the following table: $table" ; 
            carp $msg ; 
            return ( $ret , $msg ) ; 
         } 
      }
         ...
         if ( $level != 0 ) {
            $msg = "the root element should have always level = 0. Error found in the following table: $table" ; 
            carp $msg ; 
            return ( $ret , $msg ) ; 
         } 
         $lft = 1 ; $rgt = 2 ; 
