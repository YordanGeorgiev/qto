...
# };
   ...
   $cnt = "1.0.0 ::: title ::: ";
   $url              = '/' . $db . '/update/test_hi_create_table_doc' ; 
   ok ( $t->post_ok($url => json => {"attribute"=>"name", "id" =>$oid, "cnt"=>$cnt})->status_is(200), $tm);
   ...
   $tm = 'can create the 2.3.0 name - lvlalpha-0,seq-6 with siblings' ;
   $lvlalpha            = 0 ; 
   $url = '/' . $db . '/hicreate/test_hi_create_table_doc' ; 
   ok ( $t->post_ok($url => json => {'lvlalpha' => $lvlalpha, 'oid' => "$oid" })->status_is(200) , $tm );
   
   $oid = undef; # the ORIGIN id - aka the id where the add action originated ...
   $tm = 'can fetch the origin id for the 2.3.0 name' ;
   $url = '/' . $db . '/select/test_hi_create_table_doc?pick=id&with=seq-eq-6';
   ok($t->get_ok($url )->status_is(200),$tm) ; $res = $ua->get($url)->result->json ; 
   foreach my $row (@{$res->{'dat'}}){ $oid = $row->{'id'} };
   
   $tm  = 'can set the 2.3.0 element ' ; 
   $url = '/' . $db . '/update/test_hi_create_table_doc' ; 
   $cnt = "2.3.0 ::: title ::: ";
   ok ( $t->post_ok($url => json => {"attribute"=>"name", "id" =>$oid, "cnt"=>$cnt})->status_is(200), $tm);
   ...
   $tm = 'can create 3.1.0 name - lvlalpha-2,seq-8 with siblings' ;
   $lvlalpha            = 1 ; 
   $url = '/' . $db . '/hicreate/test_hi_create_table_doc' ; 
   ok ( $t->post_ok($url => json => {'lvlalpha' => $lvlalpha, 'oid' => "$oid" })->status_is(200) , $tm );
   ...
   $tm = 'can set the 3.2.0 element ' ; 
   $url = '/' . $db . '/update/test_hi_create_table_doc' ; 
   $cnt = "3.2.0 ::: title" ;
   ok ( $t->post_ok($url => json => {"attribute"=>"name", "id" =>$oid, "cnt"=>$cnt})->status_is(200), $tm);
   
   $oid = undef; # the ORIGIN id - aka the id where the add action originated ...
   $tm = 'can fetch the origin id for the new 3.1.0 name' ;
   $url = '/' . $db . '/select/test_hi_create_table_doc?pick=id&with=seq-eq-7';
   ok($t->get_ok($url )->status_is(200),$tm) ; $res = $ua->get($url)->result->json ; 
   foreach my $row (@{$res->{'dat'}}){ $oid = $row->{'id'} };
   
   $tm = 'can create a new 3.1.0 - with siblings from parent origin';
   $lvlalpha            = 1 ; 
   $url = '/' . $db . '/hicreate/test_hi_create_table_doc' ; 
   ok ( $t->post_ok($url => json => {'lvlalpha' => $lvlalpha, 'oid' => "$oid" })->status_is(200) , $tm );
   ...
   $tm = 'can set the name for 3.1.0 element ' ; 
   $url = '/' . $db . '/update/test_hi_create_table_doc' ; 
   $cnt = "3.1.0 ::: title ::: ";
   ok ( $t->post_ok($url => json => {"attribute"=>"name", "id" =>$oid, "cnt"=>$cnt})->status_is(200), $tm);
   ...
   $url = '/' . $db . '/select/test_hi_create_table_doc?pick=id&with=seq-eq-10';
   ok($t->get_ok($url )->status_is(200),$tm) ; $res = $ua->get($url)->result->json ; 
   foreach my $row (@{$res->{'dat'}}){ $oid = $row->{'id'} };
  ...
  foreach my $row (@{$res->{'dat'}}){ $oid = $row->{'id'} };
   ...
   $oid = undef; # the ORIGIN id - aka the id where the add action originated ...
   $tm = 'can fetch the origin id for the 3.1.2 name' ;
   $url = '/' . $db . '/select/test_hi_create_table_doc?pick=id&with=seq-eq-12';
   ok($t->get_ok($url )->status_is(200),$tm) ; $res = $ua->get($url)->result->json ; 
   foreach my $row (@{$res->{'dat'}}){ $oid = $row->{'id'} };
